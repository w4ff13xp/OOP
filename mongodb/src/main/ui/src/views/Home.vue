    ...
    <div class="mb-4">
      ...
      <div class="table-responsive p-0">
        <table class="table align-items-center mb-0" id="homeTable">
          <thead>
            <tr>
              <th
                class="col-2 text-uppercase text-xs text-blue font-weight-bolder opacity-7"
                @click="sortTable(0)"
              >
                Vendor
              </th>
              ...
              <th
                class="col-2 text-uppercase text-xs font-weight-bolder opacity-7 ps-2"
                @click="sortTable(3)"
              >
                Deadline
              </th>
              <th
                class="col-2 text-uppercase text-xs text-green font-weight-bolder opacity-7"
                @click="sortTable(4)"
              >
                Status
              </th>
              ...
              <td class="text-sm text-wrap fs-6 col-2">
                <div class="mx-auto mt-2">
                  <button
                    ...
                    @click="($event) => pass(h.formCode, h.formName)"
                  >
                  ...
                  <p
                    v-if="
                      this.user_access == 'Vendor' &&
                      (h.status == 'approved' || h.status == 'pendingApproval')
                    "
                  >
                  ...
                  <button
                    ...
                    v-if="
                      h.status == 'incomplete' && this.user_access == 'Admin'
                    "
                    @click="
                      email(h.formCode, h.companyName, h.formName, h.deadline)
                    "
                  >
      ...
      <nav aria-label="Page navigation example">
        <ul class="pagination">
          ...
          <li
            class="page-item"
            v-for="(each_chunk, index) in forms_chunked"
            :key="index"
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-body">
              <p>
                Are you sure you want to delete <b>{{ formName }}</b> from
                <b>{{ formCode }}</b
                >?
              </p>
            </div>
  ...
  methods: {
    ...
    setPagination() {
      ...
      if (this.user_access == "Approver") {
        for (var f in this.allForms) {
          if (
            this.allForms[f]["status"] == "pendingApproval" ||
            this.allForms[f]["status"] == "approved" ||
            this.allForms[f]["status"] == "approverRejected"
          ) {
            temp.push(this.allForms[f]);
          }
        }
      ...
      try {
        ...
        let newEmail = {
          ...
          msgBody:
            ...
            companyName +
            "," +
            "\n\nI hope this email finds you well. I am writing to remind you about the upcoming deadline for submitting the " +
            formName +
          ...
          subject:
            "Reminder: Submission of " + formName + " before " + shortDate,
        };
    ...
    async getAllForms() {
      ...
      this.user_access = JSON.parse(localStorage.getItem("specificuser"))[
        "accessRights"
      ];
      ...
      try {
        ...
        await axios
          .get("http://localhost:8080/healthEvaluation")
          .then((response) => {
            console.log(response.data);
      ...
      try {
        ...
        await axios
          .get("http://localhost:8080/preEvaluation")
          .then((response) => {
            console.log(response.data);
      ...
      try {
        ...
        await axios
          .get("http://localhost:8080/vendorAssessment")
          .then((response) => {
            console.log(response.data);
    ...
    deleteForm(formName, formCode) {
      ...
      if (formName == "Performance Evaluation Form") {
        const axios = require("axios");
        axios
          .delete("http://localhost:8080/healthEvaluation/" + formCode)
          .then((r) => {
            console.log(r);

            // DELETE FORM CODE FROM USERDB
            axios.get("http://localhost:8080/users/" + newStr).then((r) => {
              ...
              axios
                .put("http://localhost:8080/users", this.user_obj)
                .then((response) => {
                  console.log(response.data);
                  console.log("DELETED");
                  location.reload();
      ...
      if (formName == "Pre Evaluation Form") {
        const axios = require("axios");
        axios
          .delete("http://localhost:8080/preEvaluation/" + formCode)
          .then((r) => {
            console.log(r);
      ...
      if (formName == "Vendor Assessment Form") {
        const axios = require("axios");
        axios
          .delete("http://localhost:8080/vendorAssessment/" + formCode)
          .then((r) => {
            console.log(r);
